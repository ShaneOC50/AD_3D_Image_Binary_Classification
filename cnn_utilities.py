...
class loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.num_classes == 2:           # Might need to break this down into separate things for each class
...
def add_augmentation(data_path, train_files, augm_list, ndatasets=1):
    ...
    for augmentation in augm_list:
        if augmentation != "max_data":
            ...
            max_dataset = gg(data_path + 'augmentation/' + augmentation + '/*.npy')
            for file_name in max_dataset:
                for train_file_name in train_files:
                    if f'{os.path.basename(file_name).split("_batch_")[0]}.npy' ==  f'{os.path.basename(train_file_name).split("/")[-1]}':
                       augmented_data.append(file_name) 
                       break
...
def create_folds(data_path, ntest=80, nval=80, augmentation_list=[], ndatasets=1, shuffle_data=False):
    ...
    for i in range(k):
        ...
        for all_files in all_data:
            ...
            #print(f'test += all_files[{nt*i}: {nt*(i+1)}]')
            if i == k-1:
                val += all_files[:nv]
                #print(f'val += all_files[: {nv}]')
            else:
                val += all_files[nt*(i+1): nt*(i+1) + nv]
                #print(f'val += all_files[{nt*(i+1)}: {nt*(i+1)} + {nv}]')
    ...
    #print(len(test))
    #print(f'test = all_data[0][{k*nt}:] + all_data[1][{k*nt}:]')
    val = all_data[0][nv:nv*2] + all_data[1][nv:nv*2]
    #print(len(val))
    #print(f'val = all_data[0][{nv}:{nv*2}] + all_data[1][{nv}:{nv*2}]')
...
##
# ####################################################
#   Create Folds for train OASIS/ADNI test on other  #
######################################################

def create_separate_folds(data_path, testing_set="OASIS", ntest=80, nval=80, augmentation_list=[], ndatasets=1, shuffle_data=False):
    """
    It returns a list of k folds. Each fold is a list containing training, validation and testing data.
    :param
    data_path: is the path folder containing numpy files, type=str
    ...
    for i in range(k):
        ...
        for all_files in all_data:
            if testing_set == "ADNI":
                test += [f for f in all_files if testing_set in f]
                train_files = list(set(test) ^ set(all_files))  # Gets a list of all OASIS files without any ADNI files
                while len(val) < nval:
                    if i == k-1:
                        val += train_files[:nv]
                    else:
                        val += train_files[nv*(i+1): nv*(i+1) + nv]
                train += [f for f in train_files if f not in test+val]
            else:
                test += [f for f in all_files if "_OAS" in f]
                train_files = list(set(test) ^ set(all_files))  # Gets a list of all ADNI files without any OASIS files
                if i == k-1:
                    val += train_files[:nv]
                else:
                    val += train_files[nv*(i+1): nv*(i+1) + nv]
        ...
        if augmentation_list:
            train += add_augmentation(data_path, train, augmentation_list, ndatasets)
            # Below is likely not needed, script above returns only augmented files for train set
            # if testing_set == "ADNI":
            #     train += [f for f in tmp_train if testing_set not in f]
            # else:
            #     train += [f for f in tmp_train if "_OAS" not in f]
        folds.append([train, val, test])
    test = []
    train = []
    val = []
    if testing_set == "ADNI":
        test += [f for f in all_data[0] if testing_set in f] 
        test += [f for f in all_data[1] if testing_set in f]
    else:
        test += [f for f in all_data[0] if "_OAS" in f]
        test += [f for f in all_data[1] if "_OAS" in f]

    train_files = [list(set(test) ^ set(all_data[0])), list(set(test) ^ set(all_data[1]))]
    val = train_files[0][nv:nv*2] + train_files[1][nv:nv*2]
    train = [f for f in train_files[0] if f not in test + val] + [f for f in train_files[1] if f not in test + val]
    if augmentation_list:
        train += add_augmentation(data_path, train, augmentation_list, ndatasets)
    folds.append([train, val, test])
    return folds
...
def test_model(net, config, data_loader,logfile):
    """
    ...
    for _, (x, y) in enumerate(data_loader):
        ...
        y_pred = net(x)
        acc = torch.sum(torch.max(y_pred, 1)[1] == y.long()).to(dtype=torch.float)
        tot_acc += acc
        N += x.shape[0]
        yp = torch.max(y_pred, 1)[1].cpu().detach().numpy()
        yt = y.cpu().detach().numpy()
        y_predict = np.concatenate([y_predict, yp])
        y_truth = np.concatenate([y_truth, yt])

    logfile.write(f'y_predicted: {y_predict}\n\n')
    logfile.write(f'y_truth:  {y_truth}\n\n')
    conf_matrix=confusion_matrix(y_truth,y_predict)
    df_cm = pd.DataFrame(conf_matrix / np.sum(conf_matrix,axis=1)[:,None], index=["CN", "AD"], columns=["CN", "AD"])
    logfile.write(f'{df_cm.to_string()}\n\n')
    
    return tot_acc / N

def test_training_model(net, config, data_loader):
    """
    It returns the average accuracy
    :param
    net: model network
...
def train_model(net, config, train_loader, val_loader, train_acc=False, return_prob=False, logfile=False):
    ...
    for epoch_counter in range(config.num_epochs):
        ...
        mean_accuracy = test_model(net, config, val_loader,logfile=logfile).item()
        val_time = time.time()
        # COMPUTE TRAINING ACCURACY:
        if train_acc:
            train_accuracy = test_model(net, config, train_loader, logfile=logfile).item()
