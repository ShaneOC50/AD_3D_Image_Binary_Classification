...
print(epochs)
l_r = float(sys.argv[8])
print(l_r)
batch = int(sys.argv[9])
print(batch)
initial_weights = str(sys.argv[10])
print(initial_weights)
if initial_weights == "False" or initial_weights == "false":
    initial_weights=False
test_percentage=float(sys.argv[11])
print(test_percentage)

# TL Frozen Layer Testing
num_frozen_layers = int(sys.argv[12])
...
class Configuration(object):
    def __init__(self):
        ...
        self.batch_size = batch  # bach size
        self.num_epochs = epochs  # maximum number of iterations
        self.maxerror = epochs  # maximum allowed error before applying early stopping
        self.lr = l_r  # learning rate
...
class DataParameter(object):
    def __init__(self):
        ...
        self.data_augmentation_dic = {'0': 'no_augmentation', '1': 'zoom', '2': 'shift', '3': 'rotation', '4': 'all', '5':'max_data'}
        self.data_augmentation_type = []
        if sys.argv[4]:
            self.ndatasets = int(sys.argv[4])
        else:
            self.ndatasets = int(1)
...
class NetParameter(object):
    def __init__(self):
        ...
        self.fweights = [self.out_channels[-1] * out, 2] # This might need to change to 3
        self.dropout = 0.0
...
if "ADNI" in data_path:
    logfile.write('ADNI DATASET\n')
    total_imgs = 572
elif "OASIS" in data_path:
    logfile.write('OASIS DATASET\n')
    total_imgs = 436
elif "PET" in data_path:
    if "MRI" in data_path:
        logfile.write('PET MRI Datasets\n')
        total_imgs = 539
    else:
        logfile.write('PET Datasets\n')
        total_imgs = 714
else:
    logfile.write('Combined Datasets')
    total_imgs = 1008

logfile.write(f'Dataset contains {total_imgs} original MRIs')
logfile.write('Binary Classification: CN vs AD\n')

logfile.write('Exp {}\n'.format(config.nexp))

if initial_weights:
    logfile.write(f'Using the best testing weights from OASIS training in test: {initial_weights}')
...
# test_dataset_size = int(floor(total_imgs*0.15))
# validation_dataset_size = int(floor(total_imgs*0.15))

# make it 10 folds
test_dataset_size = int(floor(total_imgs*test_percentage))
validation_dataset_size = int(floor(total_imgs*test_percentage))

# April 22, 2024 testing
folds = create_folds(data_path, ntest=test_dataset_size, nval=validation_dataset_size, augmentation_list=data_info.data_augmentation_type, shuffle_data=shuffle_data,
                     ndatasets=data_info.ndatasets)
# folds = create_separate_folds(data_path, testing_set="ADNI", ntest=test_dataset_size, nval=validation_dataset_size, augmentation_list=data_info.data_augmentation_type, shuffle_data=shuffle_data,
#                      ndatasets=data_info.ndatasets)
# folds = create_separate_folds(data_path, testing_set="OASIS", ntest=test_dataset_size, nval=validation_dataset_size, augmentation_list=data_info.data_augmentation_type, shuffle_data=shuffle_data,
#                      ndatasets=data_info.ndatasets)
k = len(folds)
...
if "ADNI" in data_path:
    ...
    lnts_cn = sum([1 for f in folds[-1][-1] if '/0_OAS1' in f])
    lnts_ad = sum([1 for f in folds[-1][-1] if '/2_OAS1' in f])
elif "PET" in data_path:
    lnts_cn = sum([1 for f in folds[-1][-1] if '/0_' in f])
    lnts_ad = sum([1 for f in folds[-1][-1] if '/2_' in f])
else:
    lnts_cn = sum([1 for f in folds[-1][-1] if '/0_ADNI' in f])
    lnts_ad = sum([1 for f in folds[-1][-1] if '/2_ADNI' in f])
    lnts_cn += sum([1 for f in folds[-1][-1] if '/0_OAS1' in f])
...
print('')
print('N folds:', k)
print('The first', k - 1, 'folds contain:')
...
if initial_weights:
    w_init = torch.load(initial_weights)
else:
    w_init = dcopy(net.state_dict())
torch.save(w_init, saver_path + 'cnn_weights_init.pt')

#print(net.parameters())
#print(summary(net,(1,73, 96, 96)))
if num_frozen_layers != 0:
    c=0
    for idx, params in enumerate(net.parameters()):
        if params.requires_grad:
            if c < num_frozen_layers:
                params.requires_grad = False
        c = c+(frozen_sides)
        if idx > 23:
            # Always leave classifier trainable
            params.requires_grad = True

logfile.write(f'{summary(net,(1, 73, 96, 96))}')
...
for i in range(len(folds)):
    ...
    test_accuracy = test_model(net, config, test_loader,logfile).item()
